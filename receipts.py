...
def _fact_text_for(op_type: str, payload: dict[str, Any], result: Any) -> tuple[str, str, dict[str, Any]]:
    ...
    if op_type in ("CREATE_VENTURE", "CREATE_RECURRING_ACTIVITY"):
        res = result if isinstance(result, dict) else {}
        return "venture", f"注册营生：{payload.get('title')}（{payload.get('cadence_kind', 'daily')}）", {
            "activity_id": res.get("id"), "cadence": payload.get("cadence_kind", "daily"), "source": payload.get("source"),
        }
    if op_type in ("UPDATE_VENTURE", "UPDATE_RECURRING_ACTIVITY"):
        res = result if isinstance(result, dict) else {}
        return "venture", f"营生更新：{res.get('title') or payload.get('activity_id')} → {payload.get('status') or '改动'}", {
