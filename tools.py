...
def life_interface(args: dict, **kwargs) -> str:
    ...
    return _run(lambda rt: rt.interface(action, owner_kind, owner_id, session_id, turn_id, **payload))



def life_context(args: dict, **kwargs) -> str:
    owner_kind, owner_id = resolve_owner(args, sender_id=kwargs.get("sender_id"))
    action = args.get("action", "policy")
    payload = {k: v for k, v in args.items() if k not in {"owner_kind", "owner", "owner_id", "agent_id", "user_id", "action"}}
    if kwargs.get("platform") and "platform" not in payload:
        payload["platform"] = kwargs.get("platform")
    return _run(lambda rt: rt.context(action, owner_kind, owner_id, kwargs.get("session_id"), kwargs.get("turn_id"), **payload))

def life_living(args: dict, **kwargs) -> str:
    owner_kind, owner_id = resolve_owner(args, sender_id=kwargs.get("sender_id"))
    action = args.get("action", "summary")
    session_id = kwargs.get("session_id") or args.get("session_id")
    turn_id = kwargs.get("turn_id") or args.get("turn_id")
    payload = {k: v for k, v in args.items() if k not in {"owner_kind", "owner", "owner_id", "agent_id", "user_id", "action", "session_id", "turn_id"}}
    return _run(lambda rt: rt.living(action, owner_kind, owner_id, session_id, turn_id, **payload))

def life_collection(args: dict, **kwargs) -> str:
    owner_kind, owner_id = resolve_owner(args, sender_id=kwargs.get("sender_id"))
    action = args.get("action", "summary")
    payload = {k: v for k, v in args.items() if k not in {"owner_kind", "owner", "owner_id", "agent_id", "user_id", "action"}}
    nested = payload.pop("payload", None)
...
def life_campaign(args: dict, **kwargs) -> str:
    ...
    action = args.get("action", "list")
    payload = {k: v for k, v in args.items() if k not in {"owner_kind", "owner", "owner_id", "agent_id", "action"}}
    return _run(lambda rt: rt.campaign(action, owner_kind, owner_id, kwargs.get("session_id"), kwargs.get("turn_id"), **payload))


def life_opinion(args: dict, **kwargs) -> str:
    """The agent's evolving opinions + self-narrative: list / narrative / record / reflect."""
    owner_kind, owner_id = resolve_owner(args, sender_id=kwargs.get("sender_id"))
    action = args.get("action", "list")
    payload = {k: v for k, v in args.items() if k not in {"owner_kind", "owner", "owner_id", "agent_id", "user_id", "action"}}
