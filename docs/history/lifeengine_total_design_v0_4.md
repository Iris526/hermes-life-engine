...
# v0.4 Addendum: Executable TruthSource Layer

v0.4 upgrades TruthSource from a Canon concept into an executable subsystem.
A TruthSource binding answers: when Agent life depends on a domain such as
weather, location, time, currency, market price, or user presence, where does
that fact come from and how fresh must it be?

## TruthSource concepts
...
Life Canon binding
  ...
  -> truth_source_reads
  -> truth_source_cache
  -> Heartbeat / Planner / Final answer context
```
...
Supported authorities in v0.4:

```text
system_clock          local runtime clock, used for time/now
fixed_setting         Canon-fixed value such as JPY
user_current_location observation-backed user-location fact, often weather
external_tool         observation-backed tool result
user_reported         observation-backed user report
narrative_simulator   explicitly narrative Agent-world fact, not external reality
unknown               unresolved
```

## Observation model

LifeEngine does not hard-code a weather vendor. Instead, the Agent can call any
available Hermes tool, user source, or connector, then record the observation:

```text
life_truth(action="observe", domain="weather", result={...}, ttl_minutes=120)
```

The observation is stored in:
...
Resolution later checks cache freshness and returns `cached`, `resolved`,
`simulated`, `requires_observation`, `cached_stale`, or `unresolved`.

## Setup-safe binding

TruthSource binding changes are Canon changes, not life events. Therefore:

```text
life_truth(action="bind", domain="weather", authority="external_tool")
```

writes CanonDraft and moves LifeEngine into setup/paused_setup. The user must
commit the Canon version before the new rule becomes active.

## Heartbeat integration

Heartbeat now records a `time` TruthSource read every tick. Bindings that set
`refresh_on_heartbeat=true` are resolved during heartbeat as well. This gives
future Planner/Autonomy modules a traceable external-fact layer without relying
on user-question-triggered lazy catch-up.

## Closed-loop impact

TruthSource resolution does not itself create life facts. It creates observed
or resolved evidence that future LifeOps can cite. A later planning/event
operation should reference the `truth_read_id` or include it in evidence so the
trace can explain:

```text
why was the shopping event postponed?
  Canon weather binding
  -> weather truth read
  -> forecast says rain
  -> RESCHEDULE_EVENT LifeOp
  -> CommitReceipt fact
```
...
```text
No life fact without LifeOps commit;
no external-world dependency without TruthSource trace.
```
