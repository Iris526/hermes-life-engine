...
# v0.9.4 Maintenance / Export / Import Hardening

v0.9.4 is a pre-1.0 maintenance micro-release. It deliberately does not add new Agent life behavior. It strengthens the long-term operational story for LifeEngine profiles that may contain months or years of Agent/User life state.
...
These tables are maintenance metadata only. They do not create life events, change Canon, mutate resources, or affect Agent narrative state.

## Export archive

`life_upgrade(action="export")` creates a self-contained profile archive:

```text
lifeengine-profile-<timestamp>-<export_id>.zip
  ├── lifeengine.db
  ├── manifest.json
  └── checksums.sha256
```

The archive uses SQLite's backup API, so it is safe with WAL-backed profile databases. The manifest includes:
...
The export is recorded in `profile_exports` and `audit_log`.

## Import and staged restore

LifeEngine does **not** overwrite the running SQLite database from inside a live Hermes plugin process. Replacing an open WAL-backed SQLite DB from a running process is unsafe.

Therefore v0.9.4 implements staged restore:

```text
inspect export
  ↓
stage import under $HERMES_HOME/lifeengine/exports/imports/<import_id>
  ↓
create pre-restore backup
  ↓
write restore_staging row with offline instructions
```

The operator must stop Hermes/gateway, copy the staged DB over the active DB, then restart Hermes and run:
...
> Maintenance tooling must not corrupt the active LifeEngine database while the Agent is running.

## Package manifest and checksum

`life_upgrade(action="package_check")` builds a package manifest over the plugin directory, excluding:
...
The manifest records file path, size, SHA-256, total byte count, and aggregate manifest hash in `package_manifests`.

## Memory index verification

`life_upgrade(action="verify_memory")` validates that workspace memories have corresponding FTS5 and sqlite-vec rows. It reports missing/extra rows and records the result in `maintenance_runs`.

`life_upgrade(action="rebuild_memory")` still rebuilds both FTS5 and sqlite-vec memory indexes.

## Large DB smoke

`life_upgrade(action="large_smoke")` creates a temporary LifeEngine DB, inserts synthetic memories, rebuilds memory indexes, verifies FTS5/sqlite-vec consistency, exports the temporary DB, then records the diagnostic result. It is bounded and does not pollute the active Agent life state.
...
hermes lifeengine upgrade exports
hermes lifeengine upgrade inspect_export --archive-path <zip>
hermes lifeengine upgrade import --archive-path <zip>
hermes lifeengine upgrade restore --archive-path <zip>
hermes lifeengine upgrade package_check
hermes lifeengine upgrade verify_memory
hermes lifeengine upgrade large_smoke
```
...
```text
/life upgrade export
/life upgrade exports
/life upgrade inspect_export <zip>
/life upgrade import <zip>
/life upgrade restore <zip>
/life upgrade package_check
/life upgrade verify
/life upgrade large_smoke
```

## v0.9.4 closure status

v0.9.4 closes the release-maintenance loop:

```text
DB backup → export archive → inspect → stage import → staged restore plan → doctor/trace verification
```

This is a necessary pre-v1.0 hardening step before API/schema freeze.
