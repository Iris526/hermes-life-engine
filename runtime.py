...
from . import context_builder, diagnostics, heartbeat_modules, ops_engine
from .diagnostics import _DoctorCheckList
...
# subprocess sequence. Each entry is (output_key, runner_function, extra_ctx_keys):
# tick() runs ``runner_function(self, owner_kind, owner_id, control, tick_id,
# trace, now, *extras)`` where each extra is pulled positionally from the tick
# context by key. tick() loops over this to build ``out``, and
# _heartbeat_partial_reasons derives its check-list from it — so adding/removing
# a heartbeat module is a one-line edit here instead of hand-syncing three
# parallel lists (the audit's "4 处平行清单" module-drift root cause). Order is
# significant: modules run and land in ``out`` in this order.
_HEARTBEAT_MODULES: list[tuple[str, Callable[..., Any], tuple[str, ...]]] = [
    ("autonomy", heartbeat_modules.run_autonomy, ("manual", "authoring")),
    ("persona_drift", heartbeat_modules.run_persona_drift, ("minutes_elapsed",)),
    ("reflection", heartbeat_modules.run_reflection, ("authoring",)),
    ("meals", heartbeat_modules.settle_meals, ()),
    ("venture", heartbeat_modules.materialize_recurring, ()),
    ("campaigns", heartbeat_modules.run_campaigns, ("authoring",)),
    ("world_evolution", heartbeat_modules.run_world_evolution, ()),
    ("inter_agent_sharing", heartbeat_modules.run_inter_agent_sharing, ()),
    ("daily_rhythm", heartbeat_modules.ensure_daily_rhythm, ()),
    ("venture_supply", heartbeat_modules.settle_supply_chain, ("authoring",)),
    ("venture_opportunities", heartbeat_modules.roll_opportunities, ()),
    ("realtime_sync", heartbeat_modules.sync_realtime_to_schedule, ()),
    ("companion", heartbeat_modules.run_companion, ("authoring",)),
    ("proactive", heartbeat_modules.run_proactive, ("authoring",)),
    ("managed_review", heartbeat_modules.run_managed_review, ("manual",)),
]
...
class LifeEngineRuntime:
    ...
    def commit_ops(self, ops: list[dict[str, Any]], owner_kind: str = "agent", owner_id: str = DEFAULT_AGENT_ID,
        ...
        return ops_engine.commit_ops(self, ops, owner_kind, owner_id, source, session_id, turn_id)
    ...
    def _record_failed_lifeops(self, owner_kind: str, owner_id: str, ops: list[dict[str, Any]], source: str,
        ...
        return ops_engine._record_failed_lifeops(self, owner_kind, owner_id, ops, source, session_id, turn_id, exc)
    ...
    def _project_completed_event_safe(self, owner_kind: str, owner_id: str, event_id: str, *,
        ...
        return ops_engine._project_completed_event_safe(
            self, owner_kind, owner_id, event_id,
            summary=summary,
            source=source,
            trace_id=trace_id,
    ...
    def _commit_ops_locked(self, ops: list[dict[str, Any]], owner_kind: str, owner_id: str,
        ...
        return ops_engine._commit_ops_locked(self, ops, owner_kind, owner_id, source, session_id, turn_id, trace, control=control)
    ...
    def _apply_op(self, owner_kind: str, owner_id: str, op_type: str, payload: dict[str, Any], source: str,
        ...
        return ops_engine._apply_op(self, owner_kind, owner_id, op_type, payload, source, canon_version, trace_id=trace_id)
    ...
    def _heartbeat_partial_reasons(self, out: dict[str, Any]) -> list[str]:
        ...
        for name in [
            "resource_recovery",
            *(out_key for out_key, _runner, _extra in _HEARTBEAT_MODULES),
            "schedule_sweep",
    ...
    def tick(self, owner_kind: str = "agent", owner_id: str = DEFAULT_AGENT_ID,
        ...
        try:
            with transaction(self.conn):
                ...
                for out_key, runner, extra_keys in _HEARTBEAT_MODULES:
                    result = runner(self, owner_kind, owner_id, control, tick_id, trace, now, *(tick_ctx[k] for k in extra_keys))
    ...
    def _settle_supply_chain_for_tick(self, owner_kind: str, owner_id: str, control: dict[str, Any],
        ...
        """兼容直接调用旧私有方法的测试和外部调试脚本。"""
        return heartbeat_modules.settle_supply_chain(self, owner_kind, owner_id, control, tick_id, trace, now, authoring)
    ...
    def maintenance(self, action: str = "install_check", owner_kind: str = "agent", owner_id: str = DEFAULT_AGENT_ID, **payload: Any) -> dict[str, Any]:
        ...
        return diagnostics.run_maintenance(self, action, owner_kind, owner_id, **payload)
    ...
    def doctor(self, owner_kind: str = "agent", owner_id: str = DEFAULT_AGENT_ID,
        ...
        """
        return diagnostics.run_doctor(self, owner_kind, owner_id, level, include_samples, write_audit)
...
"""
    ...
    def startup_check(self, owner_kind: str = "agent", owner_id: str = DEFAULT_AGENT_ID, *, source: str = "startup") -> dict[str, Any]:
        return diagnostics.run_startup_check(self, owner_kind, owner_id, source=source)
    ...
    def set_context_mount(self, owner_kind: str, owner_id: str, session_id: str | None,
        ...
        return context_builder.set_context_mount(self, owner_kind, owner_id, session_id, platform=platform, mounted=mounted)

    def context_mount_status(self, owner_kind: str, owner_id: str, session_id: str | None,
                             *, platform: str | None = None) -> dict[str, Any]:
        return context_builder.context_mount_status(self, owner_kind, owner_id, session_id, platform=platform)
    ...
    def should_mount_context_for_turn(self, session_id: str | None, turn_id: str | None = None,
        ...
        return context_builder.should_mount_context_for_turn(self, session_id, turn_id, sender_id=sender_id, platform=platform)
    ...
    def _inner_life_capsule(self, owner_kind: str, owner_id: str) -> dict[str, Any]:
        ...
        注入影响主对话。
        """
        return context_builder._inner_life_capsule(self, owner_kind, owner_id)
    ...
    def build_context_for_turn(self, session_id: str | None, turn_id: str | None, user_message: str,
        ...
        胶囊，避免上下文注入阻断正常回复。
        """
        return context_builder.build_context_for_turn(self, session_id, turn_id, user_message, sender_id=sender_id, platform=platform, model=model)
    ...
    def context(self, action: str = "policy", owner_kind: str = "agent", owner_id: str = DEFAULT_AGENT_ID,
        ...
        """
        return context_builder.context(self, action, owner_kind, owner_id, session_id, turn_id, **payload)
